...
###Now with transect transformed data: log2_hel_my_data 
```{r}
...
site_data <- log2_hel_my_data %>% filter(site_id == i) # keep only the observation of sample i
...
```{r}
site_id_distribution <- table(fish_wide$site_id,fish_wide$season )
...
meta_data <- group_trans_data[,1:5] # columns i want to keep from my data
...
log2_trans_hel_c <- decostand (helge_trans_data_c, method = "log",base = 10)
...
species_rda_c <- rda_data_complete[,20:ncol(rda_data_complete)]
...
```
```{r}
rda_results <- rda(log2_trans_hel_c ~ season + depth_category+year+ site+ Hard.Substrate +Soft.Substrate + Algae ,
                data = rda_data_complete)

rda_results
summary (rda_results)
```
log2_trans_hel_c ~ season + depth_category+ reserve+ Hard.Substrate +Soft.Substrate + Algae ,
                data = rda_data_complete
...
```
# ```{r}
# rda(log2_trans_hel_c ~  season + depth_category+ reserve+ Hard.Substrate +Soft.Substrate + Algae ,
#                 data = rda_data_complete)
# signi.Inv <- rda(log2_trans_hel_c ~  season + depth_category+ reserve+ Soft.Substrate + Algae ,
#                 data = rda_data_complete)
# summary (signi.Inv)
# ```
...
rda_results_CC <- rda(log2_trans_hel_c  ~  mean_depth + TempDiff + site +Rugosity +Curvature + season,
                data = meta_data_c)
...
pairwise<-betadisper(nmds_data, # the distance matrix
                     log2_hel_my_data$site_id) # the groups
...
```{r, results='hide'}
simper_output<-simper(log2_trans_hel, #my community data (transformed)
