...
grouped_data <- read_csv("student data/grouped.data.csv")
# Read the CSV files

#group_year_data <- read.csv(csv_path)

group_year_data<-grouped_data

group_year_data$...1<-NULL
...
real_group<-group_year_data %>% dplyr::select(5,6:ncol(group_year_data))
colnames(real_group)
%>% 
  group_by(site_id) %>% 
