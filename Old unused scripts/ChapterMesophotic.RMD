...
group_year_data <- my_data %>% 
  dplyr::select(date_id, Year_season_id, site_id, depth_category, season, "Chromis chromis":"Torquigener flavimaculosus") %>% 
...
group_trans_data <- log10_hel_my_data %>% 
  dplyr::select(date_id, site, Year_season_id, site_id, depth_category, season, "Chromis chromis":"Torquigener flavimaculosus") %>% 
...
ggplot(data = renyi_df_group) +
  aes(x = Q, y = Value, group = Year_season_id, color = factor()) +
  geom_line() +
  scale_x_continuous(limits = c(0, 10)) +
...
###RDA models A. complete epibentich +fish
```{r}
rda_data <-  my_data%>%
...
```
```{r}
rda_results <- rda(log10_trans_hel_c ~ season + depth_category+ site+ Hard.Substrate +Soft.Substrate + Algae ,
...
```
# ```{r}
# rda(log10_trans_hel_c ~  season + depth_category+ reserve+ Hard.Substrate +Soft.Substrate + Algae ,
#                 data = rda_data_complete)
# signi.Inv <- rda(log10_trans_hel_c ~  season + depth_category+ reserve+ Soft.Substrate + Algae ,
#                 data = rda_data_complete)
# summary (signi.Inv)
# ```
...
plot(rda_results,xlim = c(-2,2), 
     ...
     xlab = paste0("RDA1 (11.4) %"), 
     ylab = paste0("RDA2 (4.4%)") ) # plot RDA
...
my_data_complete   <- my_data_complete[complete.cases(my_data_complete), ]
colnames(my_data_complete)
