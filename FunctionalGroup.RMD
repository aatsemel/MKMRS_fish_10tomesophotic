...
fish_filter <- fish_filter %>% 
  ...
  #select(-species) # remove redundant species column
head(fish_filter)
# If you want to reorder the columns
# fish_long <- fish_long[, c("species", "diet", ...other columns)]

# Now, fish_long contains the diet information from traits
...
fish_long <- fish_filter %>%
  dplyr::select(-transect,-a_b,-habitat, -depth, -factor_visibility, -observer, -size,-distance, -species)  %>%
   dplyr::mutate( occ =1 ) %>% 
 dplyr::group_by(across(site_id:Diet)) %>% 
 dplyr::summarise(total_ind = sum(occ))
```
#B. Calculate  the total number of Diets (n_diets_sample)and total individuals per survey (n_ind_sample)) ALSO removing NA diet counts

```{r}
fish_long<-fish_long %>%
  dplyr::group_by(tag)%>% 
  dplyr::mutate(n_diets_sample = n_distinct(Diet),
...
```{r}
fish_wide<- spread(fish_long,Diet,total_ind,fill = 0)
head(fish_wide)
colnames(fish_wide)
...
write.csv(DepthFishdataFact, here("R.generated.Data","Diets.FactorsGrp.wis.csv"))
...
sp_matrix <- my_data[,first_species:ncol(my_data)] %>%
  select_if(~ sum(.) != 0)
meta_data <- my_data[,1:32] # columns i want to keep from my data
...
group_year_data <- my_data %>% 
  dplyr::select(date_id, Year_season_id, site_id, depth_category, season, "FC":"PK") %>% 
  dplyr::group_by(date_id, site_id,depth_category, season,Year_season_id) %>% # use this unit for analyses
  dplyr::summarise(across("FC":"PK", sum),.groups = "keep")
head(group_year_data)
colnames(group_year_data)

colnames(log10_hel_my_data)
group_trans_data <- log10_hel_my_data %>% 
  dplyr::select(date_id, site, Year_season_id, site_id, depth_category, season, "FC":"PK") %>% 
...
site_sp_matrix <- site_data[,6:11] # create species matrix
...
site_sp_matrix <- site_data[,7:12] # create species matrix
...
caves_beta_sample <- beta.sample(AC_incidence, 
            ...
            samples = 15) # how many repetitions of (10) knolls to do 
...
###individual based rarefaction NOT WORKING
...
ggplot(ind_based_rare,aes(x=Individuals,y=Richness,color = Site_id))+
  geom_line(size = 1.5)+ theme_classic() +scale_color_manual(values  = color6)
```
...
###RDA models A. complete Fish and epibenthic and factors
```{r}
rda_data <-  my_data%>%
  select(1:19,32:ncol(my_data))# columns i want to keep from my data
colnames(rda_data)
...
species_rda_c <- rda_data_complete[,20:ncol(my_data_complete)]
...
rda_results <- rda(log10_trans_hel_c ~  lat + season + depth_category+ reserve+ Hard.Substrate +Soft.Substrate + Algae ,
...
```
```{r}
rda_results_CC <- rda(log10_trans_hel_c ~  mean_depth * TempDiff + reserve +Rugosity +Curvature,
...
pairwise<-betadisper(nmds_data, # the distance matrix
                     log10_hel_my_data$season) # the groups
