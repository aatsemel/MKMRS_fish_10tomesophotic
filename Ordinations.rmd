...
###visualizing the transformed species df
```{r}
...
p2=barplot(t(helge_trans_data), main = "Hellinger transformed Fish", col = rainbow(30), ylab ="Species Composition & Total Abundance", xlab ="Site (Row / Sample)")
...
```{r eval = F}
# Set max.print option to display more rows
 # Change 1000 to the desired maximum number of rows to print
...
rows_to_remove <- c(201, 203)

my_data_col_comp <- my_data %>% 
  dplyr::select(1:20) 
my_data_col_comp <- my_data_col_comp[-rows_to_remove, ]

head (my_data_col_comp)
...
species_matrix_c <- my_data_complete[,first_species_c:ncol(my_data_complete)]
...
```
# Normalize only numeric columns in the Metadata  using dplyr
```{r}

library(dplyr)

# Assuming your data frame is named meta_data
normalized_df <- my_data_complete %>%
  dplyr::mutate(
    SD_norm = scale(SD),
    VerRelief_norm = scale(VerRelief),
    Slope_norm = scale(Slope),
    Curvature_norm = scale(Curvature),
    mean_depth_norm = scale(mean_depth),
    mean_temp_norm = scale(mean_temp),
    LogVR_norm = scale(LogVR),
    TempDiff_norm = scale(TempDiff),
    Rugosity_norm = scale(Rugosity)
  )
...
# Print the normalized data frame
histogram <- ggplot(normalized_df, aes(x = TempDiff_norm)) +
    geom_histogram( fill = "skyblue", color = "black") +
    labs(title = "Histogram", x = "TempDiff_norm", y = "Frequency") +
    theme_minimal()
histogram
```
#my_data_complete has  fish and epibenthos data a
```{r}
head(normalized_df)
# Soft.substrate was omitted because it didn't affect the model and was not significant

rda_results <- rda(log2_trans_hel_c ~ lat + mean_depth+ season+ Hard.Substrate +Soft.Substrate +
TempDiff  + Curvature+ Rugosity + Algae + reserve,
...
```
#The complete with temp and rugosity data rda
```{r}
spe.rda.signif <- rda(log2_trans_hel_c ~ lat +season+ TempDiff + Rugosity +reserve,
                data = my_data_complete) 
spe.rda.signif
...
```
אין כאן סתירה? האם ההבדל בטמפ' לא קשור לעומק? האם אפשר לבנות מודל בלי הטמפרטורה אבל עם העומק?
# One model for complete rugosity, temp_diff and mean depth  (spe.rda.signif2) another for more complete with  coverage variables (spe.rda.signif3)
```{r}

spe.rda.signif2 <- rda(log2_trans_hel_c ~season+ TempDiff + Rugosity +reserve + mean_depth,
                data = my_data_complete) 
spe.rda.signif2
...
log2_trans_helcol_comp <-  log2_trans_hel[-rows_to_remove, ]

spe.rda.signif3 <- rda(log2_trans_helcol_comp ~  Algae  + lat + depth_category+  Soft.Substrate,
                data = my_data_col_comp) 
spe.rda.signif3

# Type 2 scaling
ordiplot(spe.rda.signif2, scaling = 2)
```
```{r}
ordistep(spe.rda.signif2 ,na.action = na.omit)
```

```{r}
ordistep(spe.rda.signif3 ,na.action = na.omit)
...
scale_color_manual(values = c("lightblue", "blue", "darkorange", "brown")) 
plot.new()
#plot(rda_results, type="n") # plot RDA  display = "sites"
plot(spe.rda.signif,display = c( "species"),
     # set scaling type 
...
#plot(spe.rda.signif,display = c( "cn"))
with(my_data_complete, points(spe.rda.signif
                    , col = colvec[site_id],
                     pch = 21, bg = colvec[site_id]),cex=0.5) # add color to the scores

with(my_data_complete, legend("topright", legend = levels(site_id), bty = "n",
...
plot.new()
#plot(rda_results, type="n") # plot RDA  display = "sites"
plot(spe.rda.signif,display = c( "cn","species"),
...
#orditorp(spe.rda.signif,display = "species",choices = c( 1,2),air =1, col = "black")
...
```{r}
env_columns <- my_data %>% 
  dplyr::select(site_id, season, visibility , Algae ,Soft.Substrate , VerRelief , Slope , Curvature , depth_category , Sponge , Bryozoan)
...
```{r}
adon.results <- adonis2(nmds_data ~ site_id*sample_year* season+reserve + Rugosity + TempDiff +depth_category , 
                        data = my_data,
                        method="bray",
                        perm=999,
                        na.action =na.omit)
...
```
###CCA  
Start: log2_trans_hel_c ~ lat + mean_depth + season + Hard.Substrate +      Soft.Substrate + TempDiff + Curvature + Rugosity + Algae +      reserve 
```{r}
