...
csv_path <- here("data", "fishsurvey_subGrouper.csv")
...
kruskal.test(visibility ~ depth_category, data = Abundance)
your_data <- Abundance   %>%
  dplyr::filter(site == "SdotYam" ) %>% 
     dplyr::filter(depth_category>10)
kruskal.test(Abundance ~ site_id, data = Abundance)
kruskal.test(visibility ~ depth_category, data = Abundance)
```
...
# Data
M <- subset(DepthFishdataFact,select =c(8,14:19,20,24:27,29,30 ))
pval <- psych::corr.test(M, adjust="none")$p

# Corrplot
corrplot(cor(M), type="upper", p.mat=pval,  sig.level = 0.05,   insig="blank", 
                                               tl.pos="n")
...
corrDF <-subset(DepthFishdataFact,select =c(8,14:19,20,24:27,29,30 ))
...
AbundancePlotTR <- my_data %>%
  ...
  facet_wrap(~site_id) +
  theme_classic() +
        ylab("Fish abundance per transect")
AbundancePlotTR
```
...
AbundancePlotGr <- Group_abund %>%
  ...
  ggplot(aes(y = n_ind_pooled)) +
        ylab("Fish abundance per 1000 sqm")+
  geom_boxplot(notch=TRUE, notchwidth = 0.8) +
  facet_wrap(~site_id) +
  theme_classic() 
AbundancePlotGr
```
```{r}
AbundanceTestA <- Group_abund %>%
  dplyr::filter(site == "Achziv") %>%
  rstatix::pairwise_wilcox_test(
    n_ind_pooled ~ depth_category,
    p.adjust.method = "bonferroni"
  )
AbundanceTestA

AbundanceTestSY
AbundanceTestSYS <- Group_abund %>%
  dplyr::filter(site == "SdotYam") %>%
  rstatix::pairwise_wilcox_test(
    n_ind_pooled ~ depth_category,
    p.adjust.method = "bonferroni"
...
PGrTrans <- ggplot(bray_scores_site,aes(x = Site,
  ...
  scale_color_manual(values = color6) + theme_classic()  +theme(axis.title.x = element_blank() ) +
           labs(y = "Hellinger transformed Bray-Curtis dissimilarity")# Specify color palette
...
```
```{r}
# Step 1: Calculate effective diversity (Hill numbers)
# We'll use order q=1, which is equivalent to the exponential of Shannon entropy
GrEFDIV_matrix<-group_year_data %>%
  ungroup() %>%
  select(site_id,  "Acantholabrus palloni"  :ncol(group_year_data)) %>% arrange(site_id)
df_diversity <- GrEFDIV_matrix %>%
  group_by(site_id) %>%  # Sum abundances per sample if needed
  rowwise() %>%
  mutate(
    effective_diversity = exp(diversity(c_across(where(is.numeric)), index="shannon"))
  ) %>%
  ungroup()
df_diversity
# Step 2: Create a box plot
ggplot(df_diversity, aes(x = site_id, y = effective_diversity)) +
  geom_boxplot() +
  theme_bw() +
  labs(x = "Site ID", y = "Effective Diversity", title = "Effective Diversity by Site")
```
...
```{r}
RarePlots <- ggarrange( Rp, EffDp,
                    ncol = 2, nrow = 1, common.legend = TRUE )  # Set a common x-axis title) #labels = c("Richness ", "Effective diveristy", "True diversity")
...
eco_test <- EcoTest.sample(Grpupedsp_matrix[,-1], by = Grpupedsp_matrix$site_id, niter = 500, MARGIN=1,q=0, trace = FALSE)
