...
Lgroup_year_data <- fish_long %>% 
  ...
  dplyr::group_by(site,date_id, Year_season_id, site_id, depth_category, season, species) %>% # use this unit for analyses
  dplyr::summarise(total_gr_sp = sum(total_ind, na.rm = TRUE)) %>% 
   dplyr::group_by(site,date_id, Year_season_id, site_id, depth_category, season,total_gr_sp, species ) %>% 
   dplyr::summarise(no.species = count(species(), na.rm = TRUE)) 
...
```
```{r}
rda_results <- rda(log2_trans_hel_c ~ season + depth_category+sample_year,
...
plot(rda_results,xlim = c(-2,2), 
     ...
     ylab = paste0("RDA2 (10.6%)") ) # plot RDA
