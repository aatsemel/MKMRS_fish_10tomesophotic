...
```
#summarizing abundance WITH EMPTY TRANSECT SP
```{r}
fish_long <- fish_long %>% 
  dplyr::group_by(across(site_id:species)) %>% 
...
fish_wide<-fish_wide[,-19]
colnames(fish_wide)
#fish_wide <- fish_wide[, colSums(fish_wide != 0) > 0]

write.csv(fish_wide, here("R.generated.Data", "LatFish_data_cleanW0.csv"))
...
colnames(meta_data)
corrDF <-subset(meta_data,select =c(12,15:26))
...
Q1df$VRCent <- scale(Q1df$VerRelief, center = TRUE, scale = TRUE) #not affecting
...
ggplot(Q1df, aes(x = VRCent, y = mean_depth)) +
  geom_point()+
...
mixed.lmer <- lmer(Value ~ VRCent + mean_depth + Algae + Soft.Substrate + Lat + (1 | sample_year), data = Q1df)
 # this is how you write random component effects only the intercept : after this | symbol; assigning slope to the random variables means that you suspect that there is an interaction between the factor and the affected variable
summary(mixed.lmer)
...
```
#SHaron richness and diversity is different than SY (EcoTest p=0.06; p= 0.06) but not from Ashkelon ( p = 0.25 ;p = 0.308); Achziv and SdotYam richness and diversity are not significanly different (p=0.168, p=0.126)
```{r warning=FALSE,results='hide'}
colnames(log2_hel_my_data)
Grpupedsp_matrix<-log2_hel_my_data %>%
  ungroup() %>% 
  filter(site_id== "Ashkelon30"| site_id=="SdotYam25")%>%
  select(site_id,  "Chromis chromis" :ncol((log2_hel_my_data))) %>% arrange(site_id)
...
group_year_data <- my_data %>% 
  dplyr::select( Soft.Substrate, lat, depth_category, site_id, mean_depth,sample_year, season, Algae, Bryozoa, "Chromis chromis" :"Torquigener flavimaculosus") 
# %>% 
#   dplyr::group_by( Soft.Substrate, lat, depth_category, site_id, mean_depth,sample_year, season, Algae, Bryozoa) %>% # use this unit for analyses
#   dplyr::summarise(across("Chromis chromis" :"Torquigener flavimaculosus", sum),.groups = "keep")
...
species_rda_c <- rda_data_complete[,19:ncol(rda_data_complete)]
meta_rda_c <- rda_data_complete[,1:18] # columns i want to keep from my data
...
```
```{r}
rda_results <- rda(hel_trans_data ~ Soft.Substrate + lat +  mean_depth + Algae + Bryozoa, data = rda_data_complete)
rda_results
...
#18% of fish community structure is explained by Soft substrate and Algae cover
```{r}
...
```
# Method 1: Plot only the top N species (e.g., top 10)
#top_n <- 10
top_species <- species_scores[order(rowSums(abs(species_scores)), decreasing = TRUE)[1:top_n], ]

# Method 2: Manually select species
selected_species <- species_scores[c("Species1", "Species2", "Species3"), ]
#top_species <- species_scores[order(rowSums(abs(species_scores)), decreasing = TRUE)[1:top_n], ]
# Method 3: Use a threshold (e.g., species with total absolute score > 0.5)
# threshold <- 0.5
# selected_species <- species_scores[rowSums(abs(species_scores)) > threshold, ]
```{r}
# First, extract and filter species scores
species_scores <- scores(rda_results, display = "species")

# Select specific species
top_4 <- species_scores[c("Parupeneus forsskali", "Coris julis", "Serranus cabrilla", "Siganus rivulatus"), ]

plot(rda_results, type = "p", xlim = c(-2,2), ylim = c(-1,1),
     main = "Biplot RDA",
     xlab = paste0("RDA1 (6.9%)"), 
     ylab = paste0("RDA2 (4.6%)", cex = 4))

# Add points for sites
with(rda_data_complete, points(rda_results, col = color6[site_id], 
                               pch = 21, bg = color6[site_id], cex = 0.5))

# Add legend
with(rda_data_complete, legend("bottomright", legend = levels(site_id), 
                               bty = "n", col = color6, pch = 21, pt.bg = color6))

# Add selected species arrows
arrows(0, 0, top_4[, 1], top_4[, 2], length = 0.05, col = "red")

# Add labels for selected species
text(top_4[, 1], top_4[, 2], labels = rownames(top_4), 
...
plot(rda_results, type = "n", xlim = c(-2,2), ylim = c(-1,1),
     main = "Biplot RDA",
     xlab = paste0("RDA1 (6.9%)"), 
     ylab = paste0("RDA2 (4.6%)"),
     cex.axis = 1.2,   # Increase axis label size
     cex.lab = 1.3,    # Increase axis title size
     cex.main = 1.5)   # Increase main title size

# Add points for sites with larger size
with(rda_data_complete, points(rda_results, col = color6[site_id], 
                               pch = 21, bg = color6[site_id], cex = 1.5))

# Add legend with larger point size
with(rda_data_complete, legend("bottomright", legend = levels(site_id), 
                               bty = "n", col = color6, pch = 21, pt.bg = color6,
                               pt.cex = 1.5, cex = 1.2))

# Add selected species arrows (slightly thicker)
arrows(0, 0, top_4[, 1], top_4[, 2], length = 0.1, col = "red", lwd = 1.5)

# Add labels for selected species (larger and bolder)
text(top_4[, 1], top_4[, 2], labels = rownames(top_4), 
     col = "red", cex = 1, pos = 4, font = 2)
```

```{r}

#filtering top 4 species
...
plot(rda_results,xlim = c(-2,2), 
     ...
     main = "Biplot RDA ",
     xlab = paste0("RDA1 (6.9) %"), 
     ylab = paste0("RDA2 (4.6%)") ) # plot RDA

#with(rda_data_complete, points(rda_results,  col = color6[site_id],                     pch = 21, bg = color6[site_id]),cex=0.5) # add color to the scores

with(rda_data_complete, legend("bottomright", legend = levels(site_id), bty = "n",                       col = color6, pch = 21, pt.bg = color6)) # add legend
# 
orditorp(rda_results,display = "sites",choices = c(1, 2),air =1, col = "red") # add labels to some species
...
png("tukeyhLat_plot.png", width = 1000, height = 600, res = 100)
