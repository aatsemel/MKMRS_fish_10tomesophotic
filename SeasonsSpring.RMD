...
group_year_data <- my_data %>% 
  dplyr::select(date_id, Year_season_id, site_id, depth_category, season, first_species: ncol(my_data)) %>% 
  dplyr::group_by(date_id, site_id,depth_category, season,Year_season_id) %>% # use this unit for analyses
  dplyr::summarise(across("Balistes capriscus": "Torquigener flavimaculosus", sum),.groups = "keep")
head(group_year_data)
colnames(group_year_data)

colnames(log10_hel_my_data)
group_trans_data <- log10_hel_my_data %>% 
  dplyr::select(date_id, site, Year_season_id, site_id, depth_category, season, "Balistes capriscus":"Torquigener flavimaculosus") %>% 
...
Richp <- ggplot(sample_based_rare, aes(x = Samples, y = Richness, color = Site.id)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = color6) +
  ylim(5, 25) +  # Set y-axis range up to 25
  theme_classic() +
  theme(
    legend.title = element_blank(),  # Remove legend title
    legend.text = element_text(size = 12)  # Enlarge legend text
...
TrueDp <- ggplot(sample_based_rare,aes(x=Samples,y= True.diversity,color = Site.id))+geom_line(size = 1.2) + scale_color_manual(values = color6) +  coord_cartesian(ylim = c(2, 12))  +   # Set y-axis breaks at intervals of 4 
  theme_classic() +theme(
    legend.title = element_blank(),  # Remove legend title
    legend.text = element_text(size = 12)  # Enlarge legend text
  ) 

TrueDp <- TrueDp +   scale_y_continuous(breaks = seq(2, 12, by = 2))  # Set y-axis range up to 12
TrueDp
```
...
EffDp <- ggplot(sample_based_rare,aes(x=Samples,y= Effective.diversity,color = Site.id))+geom_line(size = 1.2) + scale_color_manual(values = color6)+
 coord_cartesian(ylim = c(3, 16)) +theme_classic() +theme(
    legend.title = element_blank(),  # Remove legend title
    legend.text = element_text(size = 12))  # Enlarge legend text
    
  
EffDp <- EffDp + 
  scale_y_continuous(breaks = seq(4, 16, by = 4)) # Set y-axis range up to 12
EffDp
```
...
RarePlots <- ggarrange( Richp, EffDp,TrueDp,
          labels = c("     Spring"),          ncol = 3, nrow = 1, common.legend = TRUE )  # Set a common x-axis title)
