...
fish_filter<-fish_data %>% filter((site=="Achziv" ))
...
PGrTrans
NTTransectBeta <- NTTransectBeta + scale_y_continuous(limits = c(0.0, 1),  
                               breaks = seq(0.0, 1, by = 0.2)) + theme(axis.title.x = element_blank(),  # Remove x-axis title
                 axis.text.x = element_blank()) +
           labs(y = "Bray-Curtis dissimilarity")

TransectBeta <- TransectBeta + scale_y_continuous(limits = c(0.0, 1),  
                               breaks = seq(0.0, 1, by = 0.2)) +   theme(axis.title.x = element_blank(),  
                                                                           axis.title.y = element_blank()  ,# Remove x-axis title
                 axis.text.x = element_blank()) +
           labs(y = "Bray-Curtis dissimilarity")

pGr <- pGr + scale_y_continuous(limits = c(0.0, 1),  
                               breaks = seq(0.0, 1, by = 0.2)) + theme(axis.title.x = element_blank(),  # Remove x-axis title
                 axis.text.x = element_blank()) +
           labs(y = "Bray-Curtis dissimilarity")

PGrTrans <- PGrTrans + scale_y_continuous(limits = c(0.0, 1),  
                               breaks = seq(0.0, 1, by = 0.2)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()  , # Remove x-axis title
                 axis.text.x = element_blank()) +
           labs(y = "Bray-Curtis dissimilarity")
plot.new()
...
```{r}
rda_data <-  my_data%>%
  select(1:19,32:ncol(my_data_complete))# columns i want to keep from my data
colnames(rda_data)
...
rda_results <- rda(log10_trans_hel_c ~  season + depth_category+ reserve+ Hard.Substrate +Soft.Substrate + Algae ,
...
plot(rda_results,xlim = c(-2,2), 
     ...
     xlab = paste0("RDA1 (13.2) %"), 
     ylab = paste0("RDA2 (3.8%)") ) # plot RDA
